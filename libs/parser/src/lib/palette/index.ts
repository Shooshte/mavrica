...
export const getImagePalette = async (url: string): Promise<ParseResult[]> => {
  const { imageData, minInclusionCount } = await resizeImage(url);
  const pixelsData = await getPixels(imageData);
